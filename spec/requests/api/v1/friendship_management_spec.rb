...
RSpec.describe 'FriendshipManagement API v1', type: :request do
  ...
  # Test suite for POST /api/v1/friendship_management/block
  describe 'POST /api/v1/friendship_management/block' do
    context "invalid request" do
      context "no parameters given" do
        before(:each) do
          post '/api/v1/friendship_management/block', params: {}
        end
    ...
    context "valid request" do
      let!(:block_params) {{ requestor: "user1@example.com", target: "user2@example.com" }}

      it "calls on the FriendshipManagement::Subscribe service" do
        expect(FriendshipManagement::Subscribe).to receive(:new).and_call_original
        post '/api/v1/friendship_management/block', params: block_params
      end

      context "requestor is not yet subscribed to the target" do
        it "creates a Subscription record with blocked flag as true" do
          expect {
            post '/api/v1/friendship_management/block', params: block_params
          }.to change(Subscription, :count).by(1)
          expect(Subscription.last.requestor.email).to eq(block_params[:requestor])
          expect(Subscription.last.target.email).to eq(block_params[:target])
          expect(Subscription.last.blocked).to eq(true)
        end
      ...
      context "requestor is already subscribed to the target" do
        let!(:user1) { create(:user, email: block_params[:requestor]) }
        let!(:user2) { create(:user, email: block_params[:target]) }
      ...
      context "given email addresses are the same" do
        let!(:block_params_2) {{ requestor: "user1@example.com", target: "user1@example.com" }}

        it "returns status code for :unprocessable_entity" do
          post '/api/v1/friendship_management/block', params: block_params_2
          expect(response).to have_http_status(:unprocessable_entity)
        end
        ...
        it "returns the correct json response" do
          post '/api/v1/friendship_management/block', params: block_params_2
          expect(JSON.parse(response.body)["success"]).to eq(false)
          expect(JSON.parse(response.body)["messages"]).to include("cannot block self!")
        end
      ...
      context "one or more of the given email addresses have invalid format" do
        let!(:block_params_3) {{ requestor: "user1@example.com", target: "invalidemail" }}

        it "returns status code for :unprocessable_entity" do
          post '/api/v1/friendship_management/block', params: block_params_3
          expect(response).to have_http_status(:unprocessable_entity)
        end
      ...
      context "one or more of the given email addresses are blank" do
        let!(:block_params_4) {{ requestor: "user1@example.com", target: "" }}

        it "returns status code for :unprocessable_entity" do
          post '/api/v1/friendship_management/block', params: block_params_4
          expect(response).to have_http_status(:unprocessable_entity)
        end
